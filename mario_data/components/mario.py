...
def right_input(arr):
    lx, ly, rx, ry = arr[0], arr[1], arr[4], arr[5]
    return ry < Y_THRESH and rx > X_THRESH_R


def left_input(arr):
    lx, ly, rx, ry = arr[0], arr[1], arr[4], arr[5]
    return ly < Y_THRESH and lx < X_THRESH_L


def jump_input(arr):
    lx, ly, rx, ry, nx, ny = arr[0], arr[1], arr[4], arr[5], arr[-2], arr[-1]
    height = ny < Y_THRESH_NOSE
    return height
...
class Mario(Entity):
    ...
    def draw(self):
        ...
        if c.camera.contains(self.rect):
            ...
            if self.flip_sprites:
                ...
                c.screen.blit(
                    sprites.tile_set_flipped, (view_pos.x, view_pos.y), flipped_sprite
                )
            else:
                c.screen.blit(
                    sprites.tile_set,
    ...
    def update(self, shared_array):
        """Get input and perform actions"""

        if not self.freeze_input:
            # Press left
            if left_input(shared_array) and not right_input(shared_array):
                self.pressed_left = True
                c.ACCELERATION = -c.MARIO_ACCELERATION
            # Press right
            elif right_input(shared_array) and not left_input(shared_array):
            ...
            # No input
            if not left_input(shared_array):
                self.pressed_left = False
            if not right_input(shared_array):
                self.pressed_right = False

            # If jump
            if jump_input(shared_array) and not self.spacebar:
                self.spacebar = True
                self.action_states.on_event("jump")
            # if not jump
            if not jump_input(shared_array):
                self.spacebar = False
    ...
    def state_events(self):
        ...
        if any(
            self.current_action_state == state
            for state in ["Move_State", "Decel_State", "Brake_State", "Idle_State"]
        ):
        ...
        if all(
            self.current_action_state != state
            for state in ["Decel_State", "Brake_State", "Crouch_State"]
        ):
            c.FRICTION = 1

        if any(
            self.current_action_state == state
            for state in ["Jump_State", "No_Jump_State"]
        ):
    ...
    def collider_collisions(self, dx, dy):
        ...
        other_collider = self.rect.check_collisions(
            level.static_colliders + level.dynamic_colliders
        )
    ...
    def check_entity_collisions(self):
        ...
        entities = self.rect.check_entity_collisions(
            level.super_mushrooms + level.enemies
        )
        ...
        for entity in entities:
            ...
            if (
                hasattr(entity, "state_machine")
                and entity.state_machine.get_state() != "Knocked_State"
            ):
                if entity.state_machine.get_state() == "Shell_State":
                    ...
                    self.pos.y + self.rect.h - self.vel.y * c.delta_time < entity.pos.y
                ):
                    ...
                    if (
                        entity.state_machine.get_state() != "Shell_State"
                        and entity.can_kill
                    ):
    ...
    class Animation:
        ...
        def grow_anim(self):
            ...
            self.current_sprite = sprites.GROW_SPRITES[
                self.grow_frames[self.anim_frame]
            ]
        ...
        def run_anim(self):
            ...
            if self.mario_size == "Small_Mario":
                self.current_sprite = sprites.SMALL_MARIO_RUN[
                    self.run_frames[self.anim_frame % 4]
                ]
            else:
                self.current_sprite = sprites.BIG_MARIO_RUN[
                    self.run_frames[self.anim_frame % 4]
                ]
            self.anim_timer += c.delta_time
            if self.anim_timer > 6 * c.delta_time:
        ...
        def shrink_anim(self):
            ...
            self.current_sprite = sprites.SHRINK_SPRITES[
                self.shrink_frames[self.anim_frame]
            ]
            self.anim_timer += c.delta_time
            if self.anim_timer > 6 * c.delta_time:
                self.anim_frame += 1
                self.anim_timer = 0
            self.new_y = self.start_height + (
                self.start_sprite_height - self.current_sprite[3]
            )
    ...
    class Jump_State(State):
        ...
        def update(self, owner_object):
            ...
            if (
                not owner_object.spacebar
                or owner_object.pos.y < owner_object.start_height - c.MAX_JUMP_HEIGHT
            ):
    ...
    class Shrink_Mario(State):
        ...
        def on_enter(self, owner_object):
            owner_object.animation.reset_anim_vars()
            owner_object.animation.start_height = owner_object.pos.y
            owner_object.animation.start_sprite_height = (
                owner_object.animation.current_sprite[3]
            )
            owner_object.freeze_movement = True
            sounds.pipe.play()
